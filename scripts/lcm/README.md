...
```bash
# 默认（从仓库根目录执行，导出到 ./docs/lcm/data）
python3 scripts/lcm/exporter.py --output-dir ./docs/lcm/data

# Dry-run（不写文件）
python3 scripts/lcm/exporter.py --output-dir ./docs/lcm/data --dry-run

# 自定义 DB 路径
python3 scripts/lcm/exporter.py \
  --db-path ~/.openclaw/lcm.db \
  --output-dir ./docs/lcm/data

# 不写 history（仅 latest.json）
python3 scripts/lcm/exporter.py --output-dir ./docs/lcm/data --no-history

# 详细输出
python3 scripts/lcm/exporter.py --output-dir ./docs/lcm/data --verbose
```
...
| `--output-dir` | （必填） | JSON 输出目录（通常是 `./docs/lcm/data`） |
...
- [`../../docs/lcm/docs/exporter-lcm-spec.md`](../../docs/lcm/docs/exporter-lcm-spec.md) — 脚本规格（详细 SQL + 测试规范）
- [`../../docs/lcm/docs/DATA_SCHEMA.md`](../../docs/lcm/docs/DATA_SCHEMA.md) — JSON 字段定义
- [`../../docs/lcm/docs/SECURITY.md`](../../docs/lcm/docs/SECURITY.md) — 脱敏策略
- [`../../docs/DASHBOARD_REGISTRY.md`](../../docs/DASHBOARD_REGISTRY.md) — Dashboard 注册表（cron 任务配置参考）
